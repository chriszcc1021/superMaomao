...
func _cmd_tilemap(params: Dictionary) -> void:
	...
	var is_tilemap_layer := node.get_class() == "TileMapLayer"
	var is_tilemap_legacy := node is TileMap
	if not is_tilemap_layer and not is_tilemap_legacy:
		_send_response({"error": "Node is not a TileMapLayer/TileMap: %s (is %s)" % [node_path, node.get_class()]})
		return

	var tilemap_layer: Object = node
	var tilemap_legacy: TileMap = node as TileMap
	var layer_index: int = int(params.get("layer", 0))
	...
	match action:
		"set_cells":
			...
			for cell in cells:
				...
				if is_tilemap_layer:
					tilemap_layer.call("set_cell", pos, source_id, atlas_coords, alt_tile)
				else:
					tilemap_legacy.set_cell(layer_index, pos, source_id, atlas_coords, alt_tile)
				count += 1
		...
		"get_cell":
			...
			var alt_tile := -1
			if is_tilemap_layer:
				source_id = int(tilemap_layer.call("get_cell_source_id", pos))
				atlas_coords = tilemap_layer.call("get_cell_atlas_coords", pos)
				alt_tile = int(tilemap_layer.call("get_cell_alternative_tile", pos))
			else:
				source_id = tilemap_legacy.get_cell_source_id(layer_index, pos)
				atlas_coords = tilemap_legacy.get_cell_atlas_coords(layer_index, pos)
				alt_tile = tilemap_legacy.get_cell_alternative_tile(layer_index, pos)
			_send_response({
		...
		"erase_cells":
			...
			for cell in cells:
				var pos: Vector2i = Vector2i(int(cell.get("x", 0)), int(cell.get("y", 0)))
				if is_tilemap_layer:
					tilemap_layer.call("erase_cell", pos)
				else:
					tilemap_legacy.erase_cell(layer_index, pos)
				count += 1
		...
		"get_used_cells":
			...
			if source_filter >= 0:
				if is_tilemap_layer:
					used = tilemap_layer.call("get_used_cells_by_id", source_filter)
				else:
					used = tilemap_legacy.get_used_cells_by_id(layer_index, source_filter)
			else:
				if is_tilemap_layer:
					used = tilemap_layer.call("get_used_cells")
				else:
					used = tilemap_legacy.get_used_cells(layer_index)
...
func _cmd_ui_theme(params: Dictionary) -> void:
	...
	for color_key: String in colors:
		var c: Dictionary = colors[color_key]
		ctrl.add_theme_color_override(color_key, Color(float(c.get("r", 0)), float(c.get("g", 0)), float(c.get("b", 0)), float(c.get("a", 1))))
		applied.append("color:" + color_key)

	# Constant overrides
	var constants: Dictionary = overrides.get("constants", {})
	for const_key: String in constants:
		ctrl.add_theme_constant_override(const_key, int(constants[const_key]))
		applied.append("constant:" + const_key)

	# Font size overrides
	var font_sizes: Dictionary = overrides.get("font_sizes", {})
	for fsize_key: String in font_sizes:
		ctrl.add_theme_font_size_override(fsize_key, int(font_sizes[fsize_key]))
		applied.append("font_size:" + fsize_key)
