...
var _anim_state: String = "wander"  # wander/sleep/love/pray/beckon/mine/heal/sick
var _building_anchor: Vector2 = Vector2(-9999, -9999)  # 分配建筑的世界坐标
var _at_anchor: bool = false
var _food_drop_timer: float = 0.0
var _food_particles: Array = []  # [{pos, vy, alpha}]
...
func _process(delta: float) -> void:
	...
	if _anim_state == "pray":
		...
		for i in _food_particles.size() - 1:
...
func _draw() -> void:
	...
	# 身体
	draw_circle(shake, 14.0, cat_color)
	# 眼睛
	_draw_eyes(shake)
	# 耳朵
	draw_circle(shake + Vector2(-10.0, -12.0), 4.5, cat_color)
	draw_circle(shake + Vector2(10.0, -12.0), 4.5, cat_color)
...
func _draw_eyes(base: Vector2) -> void:
	if _anim_state == "sleep":
		# 闭眼：两条横线
		draw_line(base + Vector2(-5.5, -2.0), base + Vector2(-2.5, -2.0), Color.BLACK, 1.5)
		draw_line(base + Vector2(2.5, -2.0), base + Vector2(5.5, -2.0), Color.BLACK, 1.5)
	else:
		draw_circle(base + Vector2(-4.0, -2.0), 2.0, Color.BLACK)
		draw_circle(base + Vector2(4.0, -2.0), 2.0, Color.BLACK)
...
func _draw_zzz(base: Vector2) -> void:
	...
	for i in 3:
		...
		var alpha := sin(t * PI)
		var y_off := -20.0 - t * 25.0
		var x_off := float(i) * 6.0 - 3.0
		draw_circle(base + Vector2(x_off + 12.0, y_off), 3.5, Color(0.8, 0.8, 0.95, alpha))
...
func _draw_praying(base: Vector2) -> void:
	...
	draw_line(base + Vector2(-6.0 + rock, 10.0), base + Vector2(0.0 + rock, 20.0), Color(0.9, 0.6, 0.3), 3.0)
	draw_line(base + Vector2(6.0 + rock, 10.0), base + Vector2(0.0 + rock, 20.0), Color(0.9, 0.6, 0.3), 3.0)
...
func _draw_beckoning(base: Vector2) -> void:
	...
	for i in 2:
		...
		var alpha := sin(t * PI)
		draw_circle(base + Vector2(22.0, -20.0 - t * 20.0), 3.5, Color(1.0, 0.85, 0.1, alpha))
...
func _pick_new_target() -> void:
	...
	if is_inside_tree() and _building_anchor.x <= -999.0:
